...
void
udp_handler(uint8_t *buff, int fd, struct sockaddr *ipaddr, socklen_t addr_len, struct data *data, struct dht *dht) {
    uint8_t offset = 0;
    ...
    if (msg_type_received == LOOKUP_MODE) {
        ...
        if (data->dhtInstance->prev_node_id == data->dhtInstance->node_id) {  //node has no predessesor, should update
            data->dhtInstance->prev_node_id = node_id_received;
            strncpy(data->dhtInstance->prev_ip, node_ip_received_str, strlen(node_ip_received_str));
        ...
        if (node_id_received !=
            data->dhtInstance->node_id) {//New node has entered DHT, which is successor of current node
            data->dhtInstance->succ_id = node_id_received;
            strncpy(data->dhtInstance->succ_ip, node_ip_received_str, strlen(node_ip_received_str));
            data->dhtInstance->succ_port = node_port_received;
        }
        ...
        if (node_is_responsible(data, node_id_received)) {
            ...
            strncpy(data->dhtInstance->prev_ip, node_ip_received_str, strlen(node_ip_received_str));
            data->dhtInstance->prev_port = node_port_received;
            if (strlen(data->dhtInstance->succ_ip) == 0) {
                data->dhtInstance->succ_id = node_id_received;
                strncpy(data->dhtInstance->succ_ip, node_ip_received_str, strlen(node_ip_received_str));
                data->dhtInstance->succ_port = node_port_received;
            }
